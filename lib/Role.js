!function() {
    ...
    module.exports = new Class({
        ...
        /**
         * returns the row restrictions for a specific entity.
         *
        ...
        , getRowRestrictions: function(entityName, restrictions) {
            ...
            if (this.rowRestrictions[entityName]) {
                Object.keys(this.rowRestrictions[entityName]).forEach(function(columnName) {
                    if (!restrictions[columnName]) restrictions[columnName] = [];

                    // add my restriciotns
                    restrictions[columnName] = restrictions[columnName].concat(this.rowRestrictions[entityName][columnName]);
                }.bind(this));
            }
        }
        ...
        , update: function(roleData) {
            ...
            // prepare row restrictions, we need the least restrictive
            // restrictions
            if (roleData.rowRestriction.length) {
                roleData.rowRestriction.forEach(function(restriction) {
                    restriction.rowRestrictionEntity.forEach(function(entity) {
                        if (!this.rowRestrictions[entity.identifier]) this.rowRestrictions[entity.identifier] = {};

                        // set the column, restricitons on the same column are
                        // treated as logical and filters
                        if (!this.rowRestrictions[entity.identifier][restriction.column]) this.rowRestrictions[entity.identifier][restriction.column] = [];

                        // store filter
                        this.rowRestrictions[entity.identifier][restriction.column].push({
                              operator  : restriction.rowRestrictionOperator.identifier
                            , type      : restriction.rowRestrictionValueType.identifier
