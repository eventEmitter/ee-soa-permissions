(function() {
	...
	module.exports = new Class({
        ...
        , getPermission: asyncMethod(function(tokens, callback) {
            ...
            if (this.permissionSetCache.has(hash)) callback(null, this.permissionSetCache.get(hash));
                ...
                Promise.all(tokens.map((token) => {
                    ...
                    set = new PermissionSet(permissions, hash, this.rateLimitManager);
        ...
        , loadToken: asyncMethod(function(token, callback) {
            ...
            if (this.db.app) {
                if (this.db.tenant) {
                    let app = query.getApp(['id', 'identifier']);
                    app.getCompany('identifier').getTenant('name');   
                    app.getRole('identifier');
                }
                else {
                    let app = query.getApp(['id', 'identifier']);
                    app.getCompany('identifier');   
                    app.getRole('identifier');
                }
            }
