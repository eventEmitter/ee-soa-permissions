    ...
    module.exports = class RateLimiter {
        ...
        flushLimits() {
            this.updatedTokens.forEach((value, token) => {
                ...
                let query = `UPDATE "${dbName}"."rateLimit" set updated = now(), "currentValue" = cast((SELECT "${dbName}"."getUpdatedRateLimitValue"("${dbName}"."rateLimit".*, cast($1 as bigint)) as "currentValue") as bigint) WHERE id in (
                    SELECT
                        "rateLimit"."id" 
                    FROM
                        "${dbName}"."rateLimit" 
                ...
                this.db.executeQuery(query, [Math.round(value), token]).catch((err) => {
