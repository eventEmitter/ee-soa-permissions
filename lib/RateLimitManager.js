    ...
    module.exports = class RateLimiter {
        ...
        flushLimits() {
            this.updatedTokens.forEach((value, token) => {
                ...
                let query = `UPDATE "${dbName}"."rateLimit" set updated = now(), "currentValue" = (SELECT "${dbName}"."getUpdatedRateLimitValue"("${dbName}"."rateLimit".*, cast(${value} as bigint)) as "currentValue") WHERE id in (
                    SELECT
                        "rateLimit"."id" 
                    FROM
                        "${dbName}"."rateLimit" 
